const TaskList = () => {
  const tasks = [{ name: "I&C 6B: ZyBook", duration: "60m" }];

  return (
    <div className="bg-light-gray rounded-2xl sm:rounded-3xl md:rounded-4xl p-3 sm:p-4 md:p-5 lg:p-6 h-full flex-col">
      <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-superdark-gray mb-1">
        Task List
      </h3>
      <p className="text-xs sm:text-sm text-dark-gray mb-0 sm:mb-1 md:mb-2">
        10/5/2025
      </p>
      <div className="h-0.5 sm:h-1 bg-gradient-to-r from-mango-orange to-strawberry-red rounded-full mb-3 sm:mb-4 md:mb-6" />

      <div className="space-y-1sm:space-y-2 md:space-y-3 flex-1 mb-1 sm:mb-2 md:mb-3">
        {tasks.map((task, i) => (
